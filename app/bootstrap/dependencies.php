...
use Iot\Controller\{
    ApiController, AuthController, UserController
};
use Iot\Repository\{
    ApiRepository, AuthRepository,  UserRepository
};
use Iot\Util\{
    Authenticator, Config, Db, Logger, Validator
};
...
$container['view'] = function ($container) {
    ...
    $view->getEnvironment()->addGlobal('appName', $container->get('settings')['app_name']);
...
$container['notFoundHandler'] = function ($container) {
    return function ($request, $response) use ($container) {
        return $container['view']->render(
            $response->withStatus(404), '404.twig'
        );
    };
};
